...
const fields = 'id, role, full_name, phone, avatar_url, notifications_enabled, is_active, created_at, updated_at';
...
async function updateNotificationPreference(id, enabled) { await db.query('UPDATE users SET notifications_enabled = ? WHERE id = ?', [enabled ? 1 : 0, id]); return findById(id); }
async function deactivate(id) { await db.query('UPDATE users SET is_active = 0 WHERE id = ?', [id]); return findById(id); }
module.exports = { findByPhone, findById, create, updateProfile, updateNotificationPreference, deactivate };
