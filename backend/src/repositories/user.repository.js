...
const fields = 'id, role, full_name, phone, is_active, created_at, updated_at';
...
async function create(data, executor = db) { const sql = 'INSERT INTO users (role, full_name, phone, password_hash, is_active) VALUES (?, ?, ?, ?, ?)'; const result = executor.execute ? (await executor.execute(sql, [data.role, data.fullName, data.phone, data.passwordHash, data.isActive ?? true]))[0] : await executor.query(sql, [data.role, data.fullName, data.phone, data.passwordHash, data.isActive ?? true]); return findById(result.insertId, executor); }
async function updateProfile(id, data) { await db.query('UPDATE users SET full_name = ?, phone = ? WHERE id = ?', [data.fullName, data.phone, id]); return findById(id); }
module.exports = { findByPhone, findById, create, updateProfile };
